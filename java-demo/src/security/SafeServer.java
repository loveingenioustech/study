 ...
 * Server would load arbitrary "Service" classes to provide services. This class
 * is an alternative program to start up a Server in a similar way. The
 * difference is that this one uses a SecurityManager and a ClassLoader to
 * prevent the Service classes from doing anything damaging or malicious on the
 * local system. This allows us to safely run Service classes that come from
 * untrusted sources.
 **/
	...
	public static void main(String[] args) {
		try {
			...
			while (i < args.length) {
				...
				Server.Service service = (Server.Service) serviceClass
						.newInstance();
